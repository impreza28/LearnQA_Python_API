...
# Первый - на попытку удалить пользователя по ID 2.
    def test_delete_user_2(self):
        ...
        with allure.step('Шаг 2: попытка удаления пользователя из шага 1'):
        ...
        with allure.step('Шаг 3: проверить, что пользователь не удален'):
...
# по ID и убедиться, что пользователь действительно удален.
    ...
    # Шаг 2: авторизоваться под пользователем шага 1
        with allure.step('Шаг 2: авторизоваться под пользователем шага 1'):
            data = {'email': email,
                    'password': password}
        ...
        with allure.step('Шаг 3: удалить пользователя шага 1'):
        ...
        with allure.step('Шаг 4: получить данные пользователя по ID и убедиться, что пользователь действительно удален'):
...
# Третий - негативный, попробовать удалить пользователя, будучи авторизованными другим пользователем
    @allure.description("Тест на удаление пользователя при авторизации другим пользователем")
    def test_delete_other_user(self):
        ...
        with allure.step('Шаг 2: авторизоваться под другим пользователем'):
